...
class Strategy3:
    def __init__(self, symbol, tic):
        self.symbol = symbol
        self.tic = tic

    def model3(self):
        ticker = yf.Ticker(self.tic)
        raw = ticker.history(period='2y', interval='1d')
        raw.columns = ['Open', 'High', 'Low', self.symbol, 'Volume', 'Dividends', 'Stock Splits']

        SMA1 = 2
        SMA2 = 10
        data1= pd.DataFrame(raw[self.symbol])
        data1.columns = [self.symbol]
        data1['SMA1'] = data1[self.symbol].rolling(SMA1).mean()
        data1['SMA2'] = data1[self.symbol].rolling(SMA2).mean()
        ...
        data1['Returns'] = np.log(data1[self.symbol] / data1[self.symbol].shift(1))
        ...
        data = pd.DataFrame(raw[self.symbol])
