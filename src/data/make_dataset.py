...
class Get_Historical_Data(object):
    ...
    def getData_M(self, name, save_name='sample_'):
        new_path = self.path + 'raw/' + save_name + 'data_' + self.period + '_' + self.inter + '.pkl'
        hist = yf.download(self.tics, period=self.period, interval=self.inter)['Adj Close']
        hist.to_pickle(new_path)
        print('\n     * * * Historical Price Data Web Scrape Complete * * *\n')
    ...
    def getData_I(self, name, save_name=[
        ...
        for r in range(len(names)):
            if names[r] in name:
                new_name = self.path + 'tickers/' + save_name[r]
    ...
    def getData_I_tics(self):
        # sp500 = tickers_sp500()
        dow, other, nasdaq = tickers_dow(), tickers_other(), tickers_nasdaq()
        index_ticker_lst = [dow, other, nasdaq]
